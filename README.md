...
> 위 이미지는 키 없이 재현 가능한 `FIXTURE UI` 캡처입니다. 실제 Electron 런타임은 KIS 연결 실패 시 합성 가격으로 대체하지 않고 빈 상태 또는 오류를 표시합니다.
...
| KIS 순위 대기 | 뉴스·공시 대기 |
| --- | --- |
| ![KIS 순위 빈 상태](docs/images/paperflow-rankings.png) | ![뉴스 공시 빈 상태](docs/images/paperflow-news.png) |

키가 없거나 공급자가 아직 응답하지 않은 페이지는 `0`이나 임의의 종목을 만들지 않습니다. 이 빈 상태는 합성 데이터가 실데이터처럼 보이는 문제를 막기 위한 제품 정책입니다.
