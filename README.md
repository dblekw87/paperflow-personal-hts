...
- 공매도 카드는 KRX 정보데이터시스템 `MDCSTAT30101` CSV로 거래대금·비중을 표시하고, 잔고·대차잔고는 전용 payload 확인 전 `미제공`으로 유지
...
| 검증 | TypeScript strict, Vitest 67개 파일·358개 테스트, SQLite/금지 endpoint 헬스체크 |
...
- 전체 시장 CSV에서 현재 선택 종목 코드를 찾아 공매도 거래량, 거래대금,
  거래비중만 표시
- `공매도 순보유잔고`와 `대차 정보`는 별도 KRX 화면 payload가 확인되기
  전까지 카드에서 `미제공`으로 표시
