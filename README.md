...
- 공매도 카드는 KRX 정보데이터시스템 `MDCSTAT30101` CSV로 거래대금·비중, `MDCSTAT30501` CSV로 순보유잔고를 표시하고, 대차잔고는 전용 payload 확인 전 `미제공`으로 유지
...
- `대차 정보`는 별도 KRX 화면 payload가 확인되기 전까지 카드에서
  `미제공`으로 표시
...
| 표시 | 공매도 거래대금·비중·잔고·대차잔고 카드 shell. KRX/FINRA provider 확인 전까지 `미제공`과 미연결 사유만 표시 |
