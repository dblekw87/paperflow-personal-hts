...
| 데이터 | KIS REST/WebSocket, SEC EDGAR, OpenDART, KIND, KSD/공공데이터포털, Fed/BLS/BEA, 국내 공식 지수와 선택형 시장 proxy |
...
| 검증 | TypeScript strict, Vitest 61개 파일·340개 테스트, SQLite/금지 endpoint 헬스체크 |
...
`KRX_OPENAPI_KEY`는 KRX Data Marketplace OPEN API 인증키입니다. KIND 상장일정은
우선 공식 KIND 웹/Excel 원천을 사용하고, KRX Open API에 동일 범위 서비스가 확인되면
이 키로 provider adapter를 연결합니다.
