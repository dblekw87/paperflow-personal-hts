...
| 상호작용 설계 | KIS 종목 마스터 자동완성, 순위 행에서 종목 작업공간 이동, 호가 행 좌·우 클릭으로 로컬 매도·매수, 캘린더 날짜 선택과 이벤트 상세 drill-down |
| 안전한 데이터 표현 | 분봉 거래대금처럼 공급자 의미가 불충분한 값은 `미제공`으로 표시하고 확정 인과·호재·악재를 생성하지 않음 |
| 로딩 경험 | 차트처럼 핵심 데이터가 가장 늦게 준비되는 화면은 전체 Skeleton gate를 적용하고, 캘린더·뉴스·순위 등 provider 박스도 source별 상태를 유지 |
...
- 장전 동시호가, 장마감 동시호가, 시간외 단일가, NXT, 미국 프리·정규·애프터 세션은 fresh 호가가 있으면 로컬 모의주문을 허용하고, 장마감·stale·미수신 상태만 차단
...
- 거래 화면은 차트 history가 KIS REST projection으로 준비되기 전까지 호가·뉴스·시장 지표가 먼저 도착해도 전체 Skeleton을 유지해 부분 화면이 실제 거래 가능한 상태처럼 보이지 않게 함
- 캘린더는 선택 종목과 무관한 전체 시장 일정 페이지로 동작하고, 날짜 셀은 dot/count 중심, 오른쪽 agenda와 상세 패널에서 provider·data quality·evidence ID를 노출
- 공매도 카드는 KRX 정보데이터시스템 `MDCSTAT30101` CSV로 거래대금·비중, `MDCSTAT30501` CSV로 순보유잔고를 표시하고, 대차잔고는 공식 row-level 원천 확인 전 `보류`로 유지
...
| 데이터 | KIS REST/WebSocket, KRX OpenAPI, SEC EDGAR, OpenDART, KIND, KSD/공공데이터포털, Fed/BLS/BEA, 국내 공식 지수와 선택형 시장 proxy |
| 투자자 수급 | 거래소 원천 KRX 통계 CSV provider를 우선해 종목별·전체 시장 개인/외국인/기관 수급을 투영하고, 프로그램매매 또는 KRX 실패 시 KIS 실전 읽기 전용 REST fallback을 사용 |
...
| 저장소 | 로컬 SQLite v9, 현금·주문·체결·포지션·종목별 마지막 실호가·캘린더 이벤트 projection, 재시작 복원 |
| 안전성 | KIS 실제 주문 endpoint 미구현, stale·장외 주문 이중 차단, renderer에 키·DB·Node API 미노출 |
| 검증 | TypeScript strict, Vitest 67개 파일·372개 테스트, SQLite/금지 endpoint 헬스체크 |
...
- KIS·SEC·OpenDART·KIND·거시 일정의 서로 다른 응답을 renderer가 공급자 원본 필드 없이 어떻게 소비하게 할 것인가?
...
- KRX 정보데이터시스템 `[12009] 투자자별 거래실적(개별종목)`의
  `MDCSTAT02301` CSV 다운로드를 종목별 수급 1차 원천으로 사용
- `GenerateOTP/generate.cmd`로 OTP를 받은 뒤 `download_csv/download.cmd`에
  `code`를 POST하는 KRX 통계 다운로드 client를 별도 adapter로 분리
- CSV의 `천주/백만원` 단위를 내부 projection에서는 주/원 단위로 정규화
- 필수 participant인 개인, 외국인, 기관합계 중 하나라도 빠지면 실패 처리하고
  기존 KIS 읽기 전용 수급으로 fallback
- KRX 정보데이터시스템 `[12008] 투자자별 거래실적`의 `MDCSTAT02201`
  CSV로 전체 시장 투자자별 거래실적을 표시
- 프로그램매매는 추가 payload 확인 후 같은 KRX 통계 다운로드 client에
  연결 예정

### KRX 통계 CSV 기반 공매도

- KRX 정보데이터시스템 `공매도 거래 > 종목별 공매도 거래`의
  `MDCSTAT30101` CSV 다운로드를 종목별 공매도 거래 1차 원천으로 사용
- 화면 payload의 `searchType=1`, `mktId`, `secugrpId=BC`,
  `inqCond=STMFRTSCIFDRFSSRSWBC`, `share=1`, `money=1` 조건을 adapter
  테스트로 고정
- 전체 시장 CSV에서 현재 선택 종목 코드를 찾아 공매도 거래량, 거래대금,
  거래비중만 표시
- KRX 정보데이터시스템 `공매도 순보유잔고`의 `MDCSTAT30501` CSV로
  종목별 공매도 잔고수량, 잔고금액, 잔고비중을 표시
- `대차 정보`는 FreeSIS/KOFIA의 공식 row-level 원천이 확인되기 전까지
  카드에서 `보류`로 표시

### 국내·미국 시장 이벤트 캘린더

캘린더는 선택 종목의 부가 패널이 아니라 별도 전체 시장 화면입니다. 삼성전자,
Apple, Micron처럼 어떤 종목을 열어둔 상태여도 국내·미국·글로벌 이벤트를 함께
표시하고, 종목 workspace가 필요한 경우에만 선택 종목의 시장 범위로 좁히도록
계약을 분리했습니다.

- 실적, 배당, 권리락, 증자, 액면분할·병합, 자사주, M&A, IPO, lock-up,
  FOMC, CPI, PPI, PCE, GDP, 고용, 옵션·선물만기, MSCI/Russell/ETF 리밸런싱을
  `MarketCalendarEvent`로 정규화
- 월간 grid는 긴 제목 대신 중요도 dot과 개수로 밀도를 유지하고, 날짜를 누르면
  오른쪽 agenda에서 현지 발표시각 기준으로 정렬
- 이벤트 상세 패널은 provider, source event ID, evidence ID, 원문 URL,
  data quality, metrics를 노출해 근거 없는 일정·예상치를 확정 정보처럼 보이지 않게 함
- provider 상태 패널은 Fed/BLS/BEA/OpenDART/KIND/KSD 각각의
  `수신/미설정/실패/미지원` 상태와 신규 ingest 건수를 표시
- FOMC는 Federal Reserve 공식 HTML, CPI/PPI/고용은 BLS ICS, GDP/PCE/무역수지는
  BEA release schedule, 국내 공시는 OpenDART, 신규상장은 KIND 공식 Excel/HTML
  endpoint, 권리일정은 공공데이터포털 금융위/예탁원 API로 수집
- 캘린더와 거래 화면의 provider 박스에는 Skeleton UI를 적용하고, 실제 provider가
  실패하면 합성 일정으로 섞지 않고 source 상태와 fixture/projection 구분을 표시
...
flowchart LR
    ...
    DART["OpenDART"] --> INFO
    KIND["KIND / KSD / Fed / BLS / BEA<br/>Market Calendar"] --> CAL["Calendar Adapter"]
    ...
    RUNTIME --> DB[("SQLite v9<br/>ledger · orders · fills · book snapshots · calendar events")]
    ENGINE --> DB
    DB --> RUNTIME
...
- 보유수량 초과 매도 차단
- 수수료·세금 반영
- KRX/NXT/미국의 열린 세션에서 fresh 양방향 호가가 있을 때 신규 로컬 모의주문 허용
...
| 구현 | 공매도 거래대금·비중·순보유잔고 카드. KRX 통계 CSV로 거래·잔고를 표시하고 대차잔고는 공식 원천 확인 전 `보류`로 표시 |
...
| 구현 | 국내·미국·글로벌 이벤트 캘린더, 날짜별 agenda, provider 상태, 품질 필터, 상세 evidence 표시 |
| 구현 | Federal Reserve FOMC, BLS, BEA, OpenDART, KIND 신규상장, 공공데이터포털/KSD 권리일정 calendar adapter |
| 구현 | 차트 history 준비 전 거래 화면 전체 Skeleton gate와 캘린더/provider 카드 Skeleton |
...
`DATA_GO_KR_SERVICE_KEY`는 공공데이터포털의 금융위원회/예탁원 주식권리일정정보용
인증키입니다. 개인 모의투자 용도는 무료 신청으로 사용할 수 있지만, 해당 데이터는
공공누리 2유형이라 상업 활용 시 한국예탁결제원 정보이용계약이 필요합니다.
`KRX_OPENAPI_KEY`는 KRX Data Marketplace OPEN API 인증키입니다. 현재 repo는
KRX를 일별매매정보·종목기본정보의 우선 원천으로 다루고, 수급·공매도는 계정 내
명세서에서 전용 API 또는 data product endpoint가 확인되기 전까지 KIS fallback 또는
미지원 상태로 둡니다. KIND 상장일정은 우선 공식 KIND 웹/Excel 원천을 사용합니다.
...
│  ├─ calendar/          # FOMC/BLS/BEA/KIND/KSD/OpenDART calendar adapters
...
- [국내·미국 시장 이벤트 캘린더](docs/21-market-event-calendar.md)
- [KRX provider 전략](docs/22-krx-provider-strategy.md)
...
3. KRX provider time 기반 동시호가·시간외 단일가 phase는 표시하며, fresh 호가가 있으면 로컬 모의주문을 허용합니다. 확정 auction print adapter는 아직 미연결이므로 해당 구간의 체결가는 visible book 기반 보수 추정입니다.
4. SEC form명 일부는 한국어로 정규화하지만 공시 본문 전체 자동 번역은 아직 연결하지 않았습니다. KIS 뉴스는 제공 정책상 제목만 표시될 수 있습니다.
5. 공공데이터포털/KSD 권리일정 adapter는 구현되어 있지만, 현재 발급 직후 게이트웨이에서 `401 Unauthorized`를 반환할 수 있어 provider 상태에 실패로 노출합니다.
6. 시장 맥락은 실제 제목 기반 `WATCH` 후보까지 연결됐지만, 선택 종목의 상승·하락 원인을 가격 반응 window와 공식 evidence로 검증하는 causal assessment는 아직 연결되지 않았습니다.
7. 배포용 installer, OS 암호화 secret vault, migration 전 자동 backup은 출시 전 필수 작업입니다.
