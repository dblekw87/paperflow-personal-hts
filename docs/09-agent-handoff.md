...
국내 일별매매정보·종목기본정보처럼 KRX가 공식 OpenAPI로 제공하는 통계성 데이터는 KRX provider를 우선한다. 투자자 수급과 공매도는 거래소 원천이 맞지만, 현재 공개 OpenAPI 목록에서 전용 endpoint가 확인되지 않았으므로 임의 endpoint를 만들지 않는다. 수급은 `docs/22-krx-provider-strategy.md`에 따라 KRX provider 연결 전까지 KIS 실전 읽기 전용 REST fallback으로 표시하고, 공매도는 전용 원천 확인 전 `UNSUPPORTED`다.
...
국내·미국 기업 이벤트, 거시 발표, 파생 만기, 지수·ETF 리밸런싱 일정을 `src/contracts/market-calendar.ts`의 canonical calendar event로 정규화한다. 전체 캘린더 페이지는 국내·미국·글로벌 이벤트를 모두 표시하고, 종목 workspace는 `instrumentId`의 시장에 따라 국내 종목에는 국내+글로벌, 미국 종목에는 미국+글로벌 이벤트만 기본 표시한다. 직접 관련 이벤트는 `instrumentIds`, 시장 전체 이벤트는 `affectedMarkets`로 필터링한다.

OpenDART·KIND·예탁원 권리일정·SEC·Nasdaq/NYSE corporate actions·FOMC/BLS/BEA/BOK·Cboe/CME/MSCI/FTSE Russell 등 provider는 권리 상태와 data quality를 숨기지 않는다. KIS 뉴스 제목은 빠른 탐지 신호일 뿐 `HEADLINE_ONLY`로 유지하고, consensus·실적 예상치처럼 권리 있는 집계 데이터가 없으면 surprise를 만들지 않는다. 캘린더 원본과 evidence는 불변이며 정정·취소·변경은 version 또는 `supersedesEventId`로 연결한다. 자세한 범위는 `docs/21-market-event-calendar.md`를 따른다.

연결된 provider adapter는 Federal Reserve FOMC, BLS release calendar, BEA release schedule, OpenDART filing calendar 변환, 금융위원회/예탁원 주식권리일정정보, KIND 신규상장기업현황 Excel/HTML endpoint다. Electron runtime은 `market_calendar_events`에 공식/issuer/공공데이터 이벤트를 ingest하고 provider별 수신/실패/미설정 상태를 status message에 남긴다. `DART_CRTFC_KEY`가 없으면 OpenDART는 건너뛰며, `DATA_GO_KR_SERVICE_KEY`가 없으면 예탁원 권리일정은 건너뛴다. 예탁원 권리일정은 공공누리 2유형(출처표시+상업적 이용금지)이므로 개인 모의투자 용도에서는 공공데이터포털 키로 사용하되 상업화하면 한국예탁결제원 정보이용계약이 필요하다. KIND 상장일정은 공식 KIND 웹/Excel 응답을 `KIND_KRX`/`REGULATOR_EXCHANGE`로 정규화한다.
